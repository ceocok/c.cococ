...
set_flag_value() {
	...
	' "$3" > "$tmp_file" && mv "$tmp_file" "$3"
}

remove_proxy_networks_from_config() {
	local tmp_file; tmp_file=$(mktemp)
	awk '
		/^\[\[proxy_network\]\]/ { skip = 1; next }
		/^\[/ && skip { skip = 0 }
		!skip { print }
	' "$CONFIG_FILE" > "$tmp_file" && mv "$tmp_file" "$CONFIG_FILE"
}

append_proxy_network_entry() {
	local spec="$1"
	local cidr="${spec%%->*}"
	local mapped_cidr=""
...

	{
		...
		echo "[[proxy_network]]"
		echo "cidr = \"${cidr}\""
		if [ -n "$mapped_cidr" ]; then
			echo "mapped_cidr = \"${mapped_cidr}\""
		fi
	} >> "$CONFIG_FILE"
}

prompt_subnet_proxy_settings() {
	PROXY_NETWORK_SPECS=""
	PROXY_FORWARD_BY_SYSTEM="false"

	echo
	echo -e "${BLUE}子网代理配置（可选）${NC}"
	echo "示例: 192.168.1.0/24"
	echo "多个子网可用逗号或空格分隔；网段映射示例: 192.168.1.0/24->10.1.1.0/24"
	read -p "请输入要代理的子网 (回车跳过): " proxy_networks

	proxy_networks=$(echo "$proxy_networks" | sed 's/[[:space:]]*->[[:space:]]*/->/g')
	proxy_networks="${proxy_networks//,/ }"
	proxy_networks="${proxy_networks//，/ }"
	proxy_networks="${proxy_networks//;/ }"
	proxy_networks="${proxy_networks//；/ }"

	for spec in $proxy_networks; do
		PROXY_NETWORK_SPECS="${PROXY_NETWORK_SPECS} ${spec}"
	done

	if [ -n "$PROXY_NETWORK_SPECS" ]; then
		read -p "是否交由系统内核转发并禁用内置 NAT? (y/n，默认 n): " forward_choice
		if [[ "$forward_choice" == "y" || "$forward_choice" == "Y" ]]; then
			PROXY_FORWARD_BY_SYSTEM="true"
		fi
	fi
}

apply_subnet_proxy_settings() {
	remove_proxy_networks_from_config
	set_flag_value "proxy_forward_by_system" "$PROXY_FORWARD_BY_SYSTEM" "$CONFIG_FILE"

	for spec in $PROXY_NETWORK_SPECS; do
		append_proxy_network_entry "$spec"
	done
}
...
configure_subnet_proxy() {
	check_installed || return 1
	if [ ! -f "$CONFIG_FILE" ]; then
		echo -e "${YELLOW}配置文件不存在，请先部署或加入网络。${NC}"
		return 1
	fi
	...
	if [ -n "$PROXY_NETWORK_SPECS" ]; then
		...
		echo -e "${YELLOW}已清空子网代理配置。${NC}"
	fi

	read -p "是否立即重启 EasyTier 服务使配置生效? (y/n): " restart_choice
