...
OPENRC_FILE="/etc/init.d/xray" # Alpine OpenRC 脚本路径
...
ensure_command() {
    ...
    case "$cmd" in
        ...
        ps)    # 确保 ps 命令可用，对于最小化安装的系统
               if [ "$PKG_MANAGER" = "apk" ]; then pkg_name="procps";
               elif [ "$PKG_MANAGER" = "apt" ]; then pkg_name="procps";
               else pkg_name="procps-ng"; fi ;;
        *)
...
issue_cert() {
  ...
  ~/.acme.sh/acme.sh --issue -d "$DOMAIN" --standalone $ACME_LISTEN_PARAM --keylength ec-256 || return 1
    ...
    # 动态构建 reloadcmd，确保只执行可用的命令
   local reload_cmd
   if is_systemd_system; then
       reload_cmd="systemctl restart xray"
   elif command -v rc-service >/dev/null 2>&1; then
       reload_cmd="rc-service xray restart"
   else
       reload_cmd="pkill -f xray"
   fi
  ...
  echo "➡️ 正在设置证书自动续期任务..."
  ensure_command "crontab"
  (crontab -l 2>/dev/null | grep -Fv "acme.sh --cron") | crab - >/dev/null 2>&1
...
main() {
  detect_pkg_manager
  # 脚本开始时就确保 ps 命令存在，以便 is_systemd_system 函数能正常工作
  ensure_command "ps"
