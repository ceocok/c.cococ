...
JSDELIVR_BASE="https://cdn.jsdelivr.net/gh/ceocok/c.cococ@main"
...
download_file() {
    ...
    if command -v curl >/dev/null 2>&1; then
        ...
        # 兼容 Alpine 自带的 Busybox wget
        wget -q -T 40 -t 2 -U "$UA" --header="Referer: $REFERER" -O "$out" "$remote"
        return $?
    else
        return 127
    fi
...
run_script() {
    ...
    # 优先使用 jsDelivr 加速，回退到官方 raw，最后使用 ghproxy
    local -a urls=(
        "${JSDELIVR_BASE}/${script_name}"
